...
* Origins: Personal project to learn R studio for genomic analysis (without HPC back then)
...
* April 2026 - fixed correlation models 
...
*   Adapt Expansion Index (Kacher et al., 2021; Lee et al., 2010) to Handsaker et al. dataset
...
*   Note! Unlike typical scRNA transcriptomic data covering many genes - we only need one consensus CAG length per cell nuclei - so this deduplicated dataset only takes the longest CAG length transcript per cell. (All donors are heterozygotes)
...
```r
# plot 4 models with asymptotes if asymptotes exist
...
# create plots
create_plot <- function(model, title, color, model_type) {
r2 <- round(summary(model)$r.squared, 3)

# Generate predictions for smooth line extending to 2500
# Starting at max(1, min(total_rows)) prevents division by zero in inverse model
x_min <- max(1, min(donor_stats$total_rows))
x_seq <- seq(x_min, 2500, length.out = 100)

is_power <- (model_type == "power")
...
# Evaluate and plot horizontal asymptote if applicable and != 0
if (model_type == "inverse") {
asym_y <- coef(model)[1]
if (abs(asym_y) > 1e-9) {
p <- p + geom_hline(yintercept = asym_y, color = "brown", linetype = "dashed", linewidth = 0.8)
}
}
...
p1 <- create_plot(m_lin, "Linear: y = a + bx", "#E41A1C", "linear")
p2 <- create_plot(m_log, "Logarithmic: y = a + b*ln(x)", "#377EB8", "log")
p3 <- create_plot(m_inv, "Inverse: y = a + b/x", "#4DAF4A", "inverse")
p4 <- create_plot(m_pow, "Power: y = ax^b", "#984EA3", "power")
...
```
<img width="1000" height="800" alt="meangapcorrelations" src="https://github.com/user-attachments/assets/c90873ab-8ed5-49dc-872d-34143f5e3de1" />
...
*   **Inverse model has highest R-sq and asymptotes to ~ 1.4 bp : most biologically plausible model**
...
*   Inverse model implications: mosaicism capture increases hyperbolically as total reads increase, then saturation sets in quickly, gradually approaching asymptote of ~1.4bp (theoretical limit 1 bp)
...
print_model_stats <- function(model, name, model_type) {
cfs <- coef(model)
cat("Model:", name, "\n")
cat("R-squared:", round(summary(model)$r.squared, 4), "\n")
cat("a (Intercept):", round(cfs[1], 6), "\n")
cat("b (Slope):", round(cfs[2], 6), "\n")
cat("Horizontal Asymptote: y =", round(cfs[1], 6), "\n")
}

print_model_stats(m_inv, "Inverse (y = a + b/x)", "inverse")
```
```r
# output stats inverse model
Model: Inverse (y = a + b/x) 
R-squared: 0.8135 
a (Intercept): 1.40606 
b (Slope): 1313.38 
Horizontal Asymptote: y = 1.40606
```
...
**Fantastic CI convergence. Let's see if we can get away with lower B for efficiency. And also check convergence of random variability for twin runs within each B value.** 
...
* also this is neuron data, in real life we would use peripheral blood samples and apply a correction factor
...
_**Future explorations w/ HPC: compare gene expression profiles across EI ranges**_
